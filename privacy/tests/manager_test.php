...
class privacy_manager_testcase extends advanced_testcase {
    ...
    public function test_get_contexts_for_userid() {
        ...
        $this->assertInstanceOf(\core_privacy\request\contextlist_collection::class, $contextlistcollection);

        ob_flush();

        // Verify we have a contextlist for the component in the collection.
        $this->assertInstanceOf(\core_privacy\request\contextlist::class,
    ...
    public function test_component_is_compliant() {
        ...
        // A core_provider plugin implementing all required interfaces (metadata\provider, request\plugin_provider).
        $this->assertTrue($mockman->component_is_compliant('mod_testcomponent'));

        // A component implementing just the \core_privacy\metadata\null_provider is compliant.
        $this->assertTrue($mockman->component_is_compliant('mod_testcomponent2'));

        // A shared provider plugin implementing all required interfaces (metadata\provider, request\plugin\subplugin_provider)
        // is compliant.
    ...
    public function test_export_user_data() {
        ...
        $approvedcontextlistcollection = new \core_privacy\request\contextlist_collection(10);
        foreach ($contextlistcollection->get_contextlists() as $contextlist) {
            $approvedcontextlist = new \core_privacy\request\approved_contextlist(new stdClass(), $contextlist->get_component(),
    ...
    public function test_delete_user_data() {
        ...
        // Create an approved contextlist.
        $approvedcontextlistcollection = new \core_privacy\request\contextlist_collection($user->id);
        foreach ($contextlistcollection->get_contextlists() as $contextlist) {
            $approvedcontextlist = new \core_privacy\request\approved_contextlist($user, $contextlist->get_component(),
                $contextlist->get_contextids());
            $approvedcontextlistcollection->add_contextlist($approvedcontextlist);
        }
