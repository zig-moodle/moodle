 ...
 *     └── request\core_data_provider
 *         └── request\core_user_data_provider
 *             └── request\plugin\provider
 *             └── request\subsystem\provider
 *         └── request\user_preference_provider
 *     └── request\shared_data_provider
 *         └── request\plugin\subsystem_provider
 *         └── request\plugin\subplugin_provider
 *         └── request\subsystem\plugin_provider
 *
 ...
 * - metadata\provider (indicating they do store personal data, and describing it)
 *
 * The manager requests metadata for all Moodle components implementing the metadata\provider interface.
 *
 ...
 *      Include: request\core_data_provider and request\shared_data_provider.
 * - request\core_data_provider:
 ...
 *      Includes: request\core_user_data_provider and request\user_preference_provider.
 * - request\core_user_data_provider:
 ...
 *      Includes: request\plugin\provider and request\subsystem\provider.
 * - request\shared_data_provider:
 ...
 *      Includes: request\plugin\subsystem_provider, request\plugin\subplugin_provider and request\subsystem\plugin_provider
 *
 * The manager only requests the export or deletion of personal data for components implementing the request\core_data_provider
 * interface or one of its descendants; request\plugin\provider, request\subsystem\provider or request\user_preference_provider.
...
class manager {
    ...
    public function component_is_compliant(string $component) : bool {
        ...
        if ($this->component_implements($component, \core_privacy\metadata\null_provider::class)) {
            return true;
        }
        // Components which store user data must implement the metadata\provider and the request\data_provider.
        if ($this->component_implements($component, \core_privacy\metadata\provider::class) &&
            $this->component_implements($component, \core_privacy\request\data_provider::class)) {
            return true;
        }
        return false;
    }
    ...
    public function get_contexts_for_userid(int $userid) : contextlist_collection {
        ...
        foreach ($this->get_component_list() as $component) {
            if ($this->component_implements($component, \core_privacy\request\core_user_data_provider::class)) {
            ...
            $contextlist = request\helper::add_shared_contexts_to_contextlist_for($userid, $contextlist);
    ...
    public function export_user_data(contextlist_collection $contextlistcollection) {
        ...
        foreach($contextlistcollection as $approvedcontextlist) {
            if (!$approvedcontextlist instanceof \core_privacy\request\approved_contextlist) {
            ...
            if ($this->component_implements($component, \core_privacy\request\core_user_data_provider::class)) {
                ...
                request\helper::export_data_for_null_provider($approvedcontextlist);
            }
        }
        ...
        foreach ($this->get_component_list() as $component) {
            ...
            if ($this->component_implements($component, \core_privacy\request\user_preference_provider::class)) {
                $this->get_provider_classname($component)::export_user_preferences($contextlistcollection->get_userid());
            }
        }

        return request\writer::with_context(\context_system::instance())->finalise_content();
    }
    ...
    public function delete_user_data(contextlist_collection $contextlistcollection) {
        ...
        foreach($contextlistcollection as $approvedcontextlist) {
            ...
            request\helper::delete_user_data($approvedcontextlist);
        }
    }
    ...
    public function delete_for_context(deletion_criteria $criteria) {
        foreach ($this->get_component_list() as $component) {
            ...
            // Delete any shared user data it doesn't know about.
           request\ helper::delete_for_context($component, $criteria);
        }
    }
    ...
    protected function component_is_core_provider($component) {
        return $this->component_implements($component, \core_privacy\request\core_data_provider::class);
    }
